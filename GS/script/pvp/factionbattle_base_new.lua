...
function tbBaseFaction:StartMelee_New()
	...
	if nTotalCount < FactionBattle.MIN_ATTEND_PLAYER or not aaArena then
		local szMsg = "Vì số lượng tham dự không đủ "..FactionBattle.MIN_ATTEND_PLAYER.." người, thi đấu môn phái sẽ không diễn ra!"
	...
	self:MsgToMapPlayer("Vòng chiến đội 1/4 sẽ bắt đầu trong 10 giây nữa.");
	self:BoardMsgToMapPlayer("Vòng chiến đội 1/4 sẽ bắt đầu trong 10 giây nữa.");
...
function tbBaseFaction:ReStartMelee_New()
	...
	nTotalCount, aaArena = self:Group2Melee();
	-- 活动状态提醒
	local szMsg = string.format("Vòng chiến đội %d/4 sẽ bắt đầu trong 10 giây nữa!", self.nMeleeCount);
	self:MsgToMapPlayer(szMsg);
...
function tbBaseFaction:SendMeleeResultMsg()
	...
	if self.nMeleeCount < FactionBattle.MELEE_COUNT then		-- 前三轮的晋级比赛结果通知
		for nPlayerId, tbInfo in pairs(tbPlayer) do
			...
			-- 玩家存在，玩家在本地图，玩家有效
			if pPlayer and pPlayer.nMapId == self.nMapId and tbInfo.bEffictive == 1 then
				local szMsgBlack = string.format("Vòng chiến đội %d/4 kết thúc, nhận được %d tích lũy", self.nMeleeCount, tbInfo.OnceScore);
				local szMsgSys = string.format("Tại Vòng chiến đội %d/4 bạn nhận được %d điểm tích lũy, là cơ sở để xếp vào vòng loại trực tiếp", self.nMeleeCount, tbInfo.OnceScore);
		...
		for k, nPlayerId in pairs(self.tbWinner) do
			if nPlayerId ~= 0 then
				...
				if pPlayer and pPlayer.nMapId == self.nMapId and tbPlayer[nPlayerId].bEffictive == 1 then
					tbTemp[nPlayerId] = 1;
					Dialog:SendBlackBoardMsg(pPlayer, "Vòng Chiến đội kết thúc, bạn vào vòng 16 Cường");
					pPlayer.Msg("Vòng Chiến đội đã kết thúc, bạn đã được vào vòng 16 Cường loại trực tiếp, hãy tiếp tục cố gắng!");
					pPlayer.Msg("Giai đoạn này bạn tích lũy được " .. tbPlayer[nPlayerId].nScore .. " điểm.");
				end
			end
		end
		for nPlayerId, tbInfo in pairs(tbPlayer) do
			...
			if not tbTemp[nPlayerId] then
				...
				-- 玩家存在，玩家在本地图，玩家有效
				if pPlayer and pPlayer.nMapId == self.nMapId and tbInfo.bEffictive == 1 then
					Dialog:SendBlackBoardMsg(pPlayer, "Vòng Chiến đội kết thúc, bạn đã bị loại");
					pPlayer.Msg("Vòng Chiến đội kết thúc, rất tiếc bạn đã bị loại, bạn có thể tiếp tục tham gia để mở bảo rương");
					pPlayer.Msg("Giai đoạn này bạn tích lũy được " .. tbPlayer[nPlayerId].nScore .. " điểm.");
				end
			end
		end
...
function tbBaseFaction:StopAMelee()
	...
	if self.nMeleeCount == FactionBattle.MELEE_COUNT then	-- 混战已经结束，现在需要的是休息，准备淘汰赛，休息过程中，顺便刷刷宝箱
		self:BoardMsgToMapPlayer("Đã phân bổ Vòng Loại trực tiếp, ấn phím ~ để xem chi tiết");
		
		self:Calc16thPlayer()
		self:CalcElimination();

		local szMsg = "Tại các võ đài, xuất hiện nhiều bảo rương!";
	...
	if nCount == 0 then-- 没有玩家在比赛了，结束比赛
		...
		if self.nTimerId and self.nTimerId ~= 0 then
			...
			if nRest and nRest > Env.GAME_FPS * 10 then
				...
				self:MsgToMapPlayer("Giai đoạn kế tiếp sẽ bắt đầu sau 10 giây");
			end
		end
	else
	...
	if szFmt then
		...
		if self.nChampionAwordTimerId and self.nChampionAwordTimerId ~= 0 then
			...
			if nTimeLen and nTimeLen >  0 then
				...
				if self:GetChampionAwordCount() < FactionBattle.CHAMPION_AWARD_COUNT then
					table.insert(tbParam, nTimeLen);
					szMsgFmt = szMsgFmt .. "<color=green>Thời gian nhận thưởng lần tới: <color><color=white>%s<color>";
				end
			end
		end
		...
		if self.nBoxTimerId and self.nBoxTimerId ~= 0 then
			...
			if nTimeLen and nTimeLen > 0 then
				...
				szMsgFmt = szMsgFmt .. "<color=green>Thời gian xuất hiện bảo rương: <color><color=white>%s<color>\n\nVị trí xuất hiện: Xung quanh các võ đài";				
			end
		end
	end
...
function tbBaseFaction:EliminationAword()
	...
	local szMsg = "Có nhiều bảo rương xuất hiện, hãy nhanh tay thu thập";
...
function tbBaseFaction:FlushChampionAword(pPlayer, nCount)
	local szMsg = "Quán quân là <color=yellow>".. pPlayer.szName .. "<color>. Có rất nhiều rương xuất hiện tại.";
	self:BoardMsgToMapPlayer(szMsg);
	self:MsgToMapPlayer(szMsg);
...
function tbBaseFaction:ChampionAwordCoolDown(nPlayerId)
	...
	if pPlayer then
		...
		if self:GetChampionAwordCount() + 1 == FactionBattle.CHAMPION_AWARD_COUNT then
			szMsg = "Quán quân có thể nhận phần thưởng cuối cùng, bấm vào cờ để nhận"
		else	
			szMsg = "Quán quân có thể phát động chia phần thưởng, hãy phát lệnh.";
		end
...
function tbBaseFaction:ChampionAword_New(pPlayer)
	...
	if self.nChampionAwordTimerId ~= 0 then	-- ("每次领取奖励后需要三十秒，刚得冠军，你不觉得累吗~~搞笑~~");
		...
		if nRest and nRest > 1 then
			szMsg = string.format("Thời gian nhận thưởng kế tiếp còn %d giây", nRest);
...
function tbBaseFaction:UpdateEliminationPlayerTimer(nPlayerId, bShowMsg)
	...
	if FactionBattle.FACTIONBATTLE_MODLE == FactionBattle._MODEL_OLD then
		...
		szTimeFmt = string.format("<color=green>%s Thời gian còn lại: <color>", szN);
	else
		if self.nEliminationCount < 3 then
			szTimeFmt = string.format("<color=green>Vòng loại %d vào %d:<color>", 16 / self.nEliminationCount, 8 / self.nEliminationCount);
		elseif self.nEliminationCount == 3 then
			szTimeFmt = "<color=green>Bán kết<color>";
		elseif self.nEliminationCount == 4 then
			szTimeFmt = "<color=green>Chung kết<color>";
		end
	end
...
function tbBaseFaction:UpdatePlayerTimer_SignUp(nPlayerId)
	local	szTimeFmt = "<color=green>Thời gian đăng ký: <color><color=white>%s<color>";
...
function tbBaseFaction:UpdatePlayerTimer_MeleeRest(nPlayerId)
	...
	if self.nMeleeCount and self.nMeleeCount < 4 then
		szTimeFmt = string.format("<color=green>Vòng Chiến đội %d/4 còn:<color>", self.nMeleeCount);
	end
...
function tbBaseFaction:UpdatePlayerTimer_ReadyElimination(nPlayerId)
	...
	if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
		if not self.nEliminationCount or self.nEliminationCount == 0 then
			szTimeFmt, tbParam = self:GetPlayerTimerState("<color=green>Vòng Chiến đội kết thúc: <color><color=white>%s<color>");
		else
			...
			if nCount < 3 then
				szFmt = string.format("Thời gian nghỉ ngơi Vòng loại %d vào %d: ", 16 / nCount, 8 / nCount);
			elseif nCount == 3 then
				szFmt = "Thời gian nghỉ ngơi Bán kết: ";
			elseif nCount == 4 then
				szFmt = "Thời gian trao giải: ";
			end
		...
		local nRestTime = Timer:GetRestTime(self.nTimerId);
		szTimeFmt = "<color=green>Thời gian nghỉ ngơi: <color><color=white>%s<color>"
...
function tbBaseFaction:UpdatePlayerTimer_ChampionAword(nPlayerId)
	...
	if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
		szTimeFmt, tbParam = self:GetPlayerTimerState("<color=green>Giai đoạn trao giải: <color><color=white>%s<color>");
		self:UpdatePlayerTimer_Detail("", szTimeFmt, tbParam, nPlayerId, 0);
	else
		szTimeFmt = "<color=green>Thời gian nhận giải: <color><color=white>%s<color>";
		self:UpdatePlayerTimer_Detail("", szTimeFmt, tbParam, nPlayerId, 0);
	end
end
