...
function tbGirl:AttendDailyEvent(nType, bOk)
	...
	if nAttendTime < 0 then
		Dialog:Say("由于您的违规操作，您的美女认证资格已经被取消。", {{"那就去网站看看美女吧 ", self.UpPic,self},{"Kết thúc đối thoại"}});
		return 0;
	end
	if me.nSex ~= Env.SEX_FEMALE then
		Dialog:Say("只有美女才可以参加认证，你这小子，是想糊弄本姑娘吗？", {{"那就去网站看看美女吧 ", self.UpPic,self},{"Kết thúc đối thoại"}});
		return 0;
	end
	if nLastTime > 0 and self.nGirlLogoTime + nLastTime > GetTime() then
	...
	if nAttendTime > 0 then
		Dialog:Say("恭喜你，已成功报名！<enter>您的资料正在上传，请于报名<color=green>24小时后前往官网<color>补填资料、上传个人照片方可获得最终认证。", {{"去官网看看 ", self.UpPic,self},{"Kết thúc đối thoại"}});
		return 0;
	end
	...
	if #tbFind <= 0 then
		Dialog:Say("您的包裹内没有[海洋之心碎片]，无法进行美女认证。<enter><color=yellow>是否要花费2999金币购买一个[海洋之心碎片]<color>？", {{"购买碎片",self.BuyItem, self},{"Ta chỉ xem qua thôi"}});
		return;
	end
	if #tbFind > 1 then
	...
	if not bOk then
		Dialog:Say("您确认报名参加美女认证活动，将扣除您身上的<color=yellow>海洋之心碎片<color>？", {{"是的，我要参加",self.AttendDailyEvent, self, nType, 1},{"Để ta suy nghĩ thêm"}});
		return;
	end
