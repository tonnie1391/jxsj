...
function DragonBoat:OnPlayerBaseType1(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("Phát hiện ra Bí Thuật %s dưới lòng sông!", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>Phát hiện ra Bí Thuật %s dưới lòng sông!<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType2(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("Thuyền Rồng đang bị Bí Thuật %s quấy nhiễu!", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>Thuyền Rồng đang bị Bí Thuật %s quấy nhiễu!<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType3(nType, nGroup, nPoint)
	...
	if szItemName then
		Dialog:SendBlackBoardMsg(me, string.format("Vô tình nhặt được %s dưới lòng sông!", szItemName));
		me.Msg(string.format("<color=blue>Vô tình nhặt được %s dưới lòng sông!<color>", szItemName));
	end
end
...
function DragonBoat:OnPlayerBaseType4(pCastPlayer, tbPlayerInfo)
	...
	if pCastPlayer.nId == pPlayer.nId then
		local szMsg = string.format("Thật xui xẻo, ngươi bị Thiên Phạt %s tự mình công kích", KFightSkill.GetSkillName(tbSkill[1]));
		pPlayer.Msg(string.format("<color=blue>%s<color>", szMsg));
		Dialog:SendBlackBoardMsg(pPlayer, szMsg);
		return 0;
	end
	
	local szMsg1 = string.format("Thật đáng ghét, ngươi bị %s dùng Thiên Phạt %s công kích!", pCastPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
	local szMsg2 = string.format("Vừa tấn công %s bằng Thiên Phạt %s!", pPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
