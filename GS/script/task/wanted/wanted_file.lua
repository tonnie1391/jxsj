...
function Wanted:ReadMainTask()
	...
	for nTaskId, tbSubFile in pairs(self.TaskFile) do
		...
		tbReferData.tbDesc.tbStepsDesc[1] = string.format("Hình bộ nha môn phát công văn thông báo, Hải tặc <pos=%s,%s,%s,%s> gần đây xuất hiện quanh <color=yellow>%s<color=white>. Bạn cần phải bắt chúng về quy án!",tbSubFile.szTaskName,tbSubFile.nMapId, tbSubFile.nPosX, tbSubFile.nPosY, tbSubFile.szMapName);
