...
function Tong:PerTongDailyAction_Timer()
	...
	if cTong.GetTestState() ~= 0 then
		if cTong.GetBuildFund() >= self.TEST_PASS_BUILD_FUND then
			...
			KTong.Msg2Tong(nTongId, "Chúc mừng, Bang hội đã vượt qua khảo nghiệm!")
		elseif (EventManager.IVER_bOpenTiFu == 1) then
			...
			if nLeftTime < 0 then
				KTong.Msg2Tong(nTongId, "Quỹ xây dựng chưa đạt "..self.TEST_PASS_BUILD_FUND.." trong thời gian khảo nghiệm, Bang hội tự giải tán!")
				Tong:DisbandTong_GC(nTongId);
				cTong = nil;	-- 对象失效，赋予nil值
			else
				KTong.Msg2Tong(nTongId, "Trong <color=red>"..math.ceil(nLeftTime / 24 / 3600)..
				"<color> ngày khảo nghiệm, quỹ xây dựng Bang hội phải đạt "..self.TEST_PASS_BUILD_FUND..", nếu không sẽ tự giải tán.")
			end
		end
	end
	-- 如果帮会已经没有家族则解散帮会
