...
function XoyoChallenge:GetAward(pPlayer)
	...
	if 1 <= nRank and nRank <= 100 then
		if nCanGetKinAward == 1 then
			pPlayer.SendMsgToFriend(string.format("您的好友[%s]在上个月的逍遥录收集任务中排名第%d位，家族的地狱逍遥积分排名前10，获得%s奖励。",
				pPlayer.szName, nRank, szAwardName));
		else
			pPlayer.SendMsgToFriend(string.format("您的好友[%s]在上个月的逍遥录收集任务中排名第%d位，获得%s奖励。",
				pPlayer.szName, nRank, szAwardName));
		end
	end
